...
-- First verify xp_levels table exists, if not create it
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables
                   WHERE table_schema = 'public'
                   AND table_name = 'xp_levels') THEN
        RAISE NOTICE 'xp_levels table not found. Please run 0004_xp_levels_mapping.sql first.';
        RAISE EXCEPTION 'Required table xp_levels does not exist';
    END IF;
END $$;
...
-- Recreate with correct level calculation, but using a simpler approach
...
SELECT
    ...
    p.steam_id64,
    p.country,
    -- Use a subquery to calculate level directly
    COALESCE(
        (SELECT xl.level
         FROM xp_levels xl
         WHERE p.xp >= xl.xp_min
           AND p.xp <= xl.xp_max
         LIMIT 1),
        p.level,
        1
...
-- Drop the existing functions first since we're changing their return types
DROP FUNCTION IF EXISTS search_players(text, integer, integer);
DROP FUNCTION IF EXISTS autocomplete_players(text, integer);
