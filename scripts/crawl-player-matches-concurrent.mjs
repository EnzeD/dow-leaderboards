...
const RELIC_DELAY_MS = Number.parseInt(process.env.CRAWL_RELIC_DELAY_MS ?? "100", 10);
const RELIC_REQUEST_CAP = Number.parseInt(process.env.CRAWL_RELIC_REQUEST_CAP ?? "50000", 10);
const JOB_MAX_ATTEMPTS = Number.parseInt(process.env.CRAWL_JOB_MAX_ATTEMPTS ?? "5", 10);
const COOLDOWN_MINUTES = Number.parseInt(process.env.CRAWL_COOLDOWN_MINUTES ?? "60", 10);
const UPSERT_CHUNK_SIZE = Number.parseInt(process.env.CRAWL_UPSERT_CHUNK_SIZE ?? "500", 10);
const IDLE_SLEEP_MS = Number.parseInt(process.env.CRAWL_IDLE_SLEEP_MS ?? "1000", 10);
...
const CONCURRENCY = Number.parseInt(process.env.CRAWL_CONCURRENCY ?? "6", 10);
const BATCH_SIZE = Number.parseInt(process.env.CRAWL_BATCH_SIZE ?? "20", 10);
...
async function fetchRelicJson(url, context) {
  ...
  relicRequestCount += 1;
  const res = await fetch(url, { headers: { "User-Agent": "dow1-de-match-crawler-concurrent" } });
  if (!res.ok) {
...
async function claimJobsBatch(batchSize) {
  // For concurrent safety, we'll claim jobs one by one in a batch
  const claimedJobs = [];

  for (let i = 0; i < batchSize; i++) {
    try {
      // Get one candidate job
      ...
      if (candidateQuery.error) {
        console.warn(`Failed to select crawl job: ${candidateQuery.error.message}`);
        break;
      }
      ...
      if (updatedQuery.error) {
        ...
        console.warn(`Failed to claim crawl job ${candidate.id}: ${updatedQuery.error.message}`);
        continue;
      }

      claimedJobs.push(updatedQuery.data);
    } catch (error) {
      console.warn(`Error claiming job: ${error.message}`);
      break;
    }
  }
...
async function recordRunStart(jobId, startedAt, alias) {
  ...
  if (insert.error && insert.error.code !== "23505") {
    console.warn(`Failed to record run start for job ${jobId}:`, insert.error.message);
  }
}
...
async function finalizeRun(jobId, startedAt, payload) {
  ...
  if (update.error) {
    console.warn(`Failed to update run for job ${jobId}:`, update.error.message);
  }
}
...
async function enqueueProfiles(profileIds, playerMap) {
  for (const profileId of profileIds) {
    ...
    if (insert.error && !isDuplicateError(insert.error)) {
      console.warn(`Failed to enqueue profile ${profileId}:`, insert.error.message);
    }
  }
}
...
async function updateJobStatus(jobId, payload) {
  const update = await supabase
    ...
    .eq("id", jobId);
  if (update.error) {
    console.warn(`Failed to update crawl job ${jobId}:`, update.error.message);
  }
}
...
async function runWorker(workerId) {
  while (true) {
    try {
      const jobs = await claimJobsBatch(BATCH_SIZE);
      if (!jobs.length) {
      ...
      const results = await Promise.all(jobs.map(job => handleJob(job)));

      const succeeded = results.filter(r => r.success).length;
      const failed = results.filter(r => !r.success).length;

      totalJobsProcessed += results.length;
      totalJobsSucceeded += succeeded;
      totalJobsFailed += failed;

      if (succeeded > 0 || failed > 0) {
        console.log(`Worker ${workerId}: Batch complete - ${succeeded} succeeded, ${failed} failed`);
      }
...
async function printStats() {
  ...
  const interval = setInterval(() => {
    ...
    const jobsPerMinute = Math.round(totalJobsProcessed / elapsedMinutes);
    const jobsPerHour = Math.round(jobsPerMinute * 60);

    console.log(`📊 Stats: ${totalJobsProcessed} jobs processed (${totalJobsSucceeded} success, ${totalJobsFailed} failed) | Rate: ${jobsPerHour}/hour | API calls: ${relicRequestCount}`);
  }, 30000); // Every 30 seconds

  // Cleanup on exit
  process.on('SIGINT', () => {
    clearInterval(interval);
    console.log("\n👋 Concurrent crawler stopped");
    console.log(`Final stats: ${totalJobsProcessed} jobs processed, ${relicRequestCount} API calls`);
    process.exit(0);
  });
}

async function main() {
  console.log(`🚀 Starting concurrent match crawler with ${CONCURRENCY} workers...`);
  console.log(`📊 Settings: ${RELIC_DELAY_MS}ms delay, batch size ${BATCH_SIZE}, cooldown ${COOLDOWN_MINUTES}m`);
  ...
  if (EXIT_ON_IDLE) {
    // In exit-on-idle mode, we need different logic
    let consecutiveIdleRounds = 0;
    while (true) {
      const jobs = await claimJobsBatch(BATCH_SIZE * CONCURRENCY);
      if (!jobs.length) {
        consecutiveIdleRounds++;
        if (consecutiveIdleRounds > 3) {
...
main().catch(err => {
  console.error("Concurrent crawler failed:", err);
  process.exit(1);
});