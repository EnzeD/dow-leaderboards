...
export async function GET(req: NextRequest) {
  ...
  const mapIdentifierRaw =
    url.searchParams.get("mapIdentifier") ?? url.searchParams.get("map_identifier");

  if (!mapIdentifierRaw || mapIdentifierRaw.trim().length === 0) {
    return attachCacheHeaders(
      NextResponse.json<MapMatchHistoryResponse>({
        activated: false,
        profileId: "",
        mapIdentifier: "",
        ...
        rows: [],
        reason: "map_identifier_required",
      }, { status: 400 })
    );
  }
  ...
  if (!session) {
    return attachCacheHeaders(
      NextResponse.json<MapMatchHistoryResponse>({
        activated: false,
        profileId: "",
        mapIdentifier: normalizeIdentifier(mapIdentifierRaw),
        windowStart: resolveSinceDate(),
        generatedAt: new Date().toISOString(),
        rows: [],
  ...
  if (appUserError) {
    console.error("[premium] map history failed to load app_user", appUserError);
    return attachCacheHeaders(
      NextResponse.json<MapMatchHistoryResponse>({
        activated: false,
        profileId: "",
  ...
  const mapIdentifier = normalizeIdentifier(mapIdentifierRaw);
  ...
  try {
    const { data, error } = await supabase.rpc("premium_get_map_match_history", {
    ...
    if (error) {
      console.error("[premium] map match history rpc failed", error);
      return attachCacheHeaders(
        NextResponse.json<MapMatchHistoryResponse>({
          activated: true,
          profileId,
    ...
    const normalizedRows: MapMatchRow[] = rows.map((row: any) => {
    ...
    console.error("[premium] map match history unexpected error", error);
    return attachCacheHeaders(
      NextResponse.json<MapMatchHistoryResponse>({
        activated: true,
        profileId,
