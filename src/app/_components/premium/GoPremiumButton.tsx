  ...
  return (
    <div className="space-y-2">
      <button
        ...
        onClick={handleClick}
        disabled={loading}
        className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-neutral-600/60 bg-neutral-800/40 px-4 py-2 text-sm font-semibold text-neutral-100 transition hover:bg-neutral-700/50 disabled:cursor-not-allowed disabled:opacity-60"
      >
        {loading ? (
          "Opening portal…"
        ) : (
          <>
            Manage <ProBadge size="xs" clickable={false} /> membership
          </>
        )}
      </button>
      {error && (
